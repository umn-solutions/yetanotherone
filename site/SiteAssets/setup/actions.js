...
export async function backfillInitiativeDates(siteApi) {
  log('--- Backfill Initiative Dates ---', 'info');

  const initiatives = await siteApi.list('Initiatives').getItems(undefined, { limit: Infinity });
  const events = await siteApi.list('InitiativeEvents').getItems(undefined, { limit: Infinity, orderBy: [{ field: 'ID' }] });

  const eventsByUUID = new Map();
  for (const ev of events) {
    const list = eventsByUUID.get(ev.InitiativeUUID) || [];
    list.push(ev);
    eventsByUUID.set(ev.InitiativeUUID, list);
  }
  ...
  for (const initiative of initiatives) {
    const evList = eventsByUUID.get(initiative.UUID) || [];
    const fields = {};

    if (!initiative.SubmittedDate) {
      const submissionEvents = evList.filter(ev => ev.EventType === 'Submission');
      if (submissionEvents.length > 0) {
        const earliest = submissionEvents.reduce((a, b) => (a.Date <= b.Date ? a : b));
        fields.SubmittedDate = earliest.Date;
      }
    }

    if (!initiative.ImplementedDate) {
      const implEvents = evList.filter(ev => ev.EventType === 'OwnerImplementation');
      if (implEvents.length > 0) {
        const latest = implEvents.reduce((a, b) => (a.Date >= b.Date ? a : b));
        fields.ImplementedDate = latest.Date;
      }
    }
    ...
    try {
      await siteApi.list('Initiatives').updateItem(initiative.Id, fields, initiative['odata.etag']);
      const parts = [];
      if (fields.SubmittedDate) { parts.push('SubmittedDate=' + fields.SubmittedDate); submittedCount++; }
      if (fields.ImplementedDate) { parts.push('ImplementedDate=' + fields.ImplementedDate); implementedCount++; }
      log('Backfilled ' + parts.join(', ') + ' on Initiative #' + initiative.Id);
    } catch (e) {
      log('Failed to backfill Initiative #' + initiative.Id + ': ' + (e.message || 'unknown error'), 'error');
    }
  }

  log('Backfill complete: ' + submittedCount + ' SubmittedDate, ' + implementedCount + ' ImplementedDate, ' + untouched + ' untouched', 'success');
}
